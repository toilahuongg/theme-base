class CartDrawer extends HTMLElement {
  constructor() {
    ...
    window.VelouraTheme.Cart = this;
    this.drawer = this.closest("drawer-component");
    window.VelouraTheme.CartDrawer = this.drawer;
  ...
  connectedCallback() {
    window.VelouraEvents.subscribe(
  ...
  async onCartRefresh(event) {
    ...
    try {
      const response = await fetch(
        `${VelouraSettings.routes.root}?section_id=cart-drawer`
      );
  ...
  renderContents(parsedState, renderFooter = true, isOpen = true) {
    ...
    setTimeout(() => {
      if (this.openWhenAdded && !this.drawer.isOpen() && isOpen) {
        ...
        window.VelouraEvents.emit(`ON_CART_DRAWER_OPEN`, parsedState);
      }
    });
  }
...
class CartDrawerItems extends CartItems {
  initLoading() {
    this.loading = new window.VelouraTheme.AnimateLoading(
