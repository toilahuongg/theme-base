window.VelouraTheme = window.VelouraTheme || {};
...
function loadAssets(files = [], id, callback = () => {}, options = {}) {
  ...
  if (!window.VelouraTheme.loadjs.isDefined(id))
    window.VelouraTheme.loadjs(files, unique);
  window.VelouraTheme.loadjs.ready(unique, callback);
}
...
class SliderComponent extends HTMLElement {
  ...
  init() {
    ...
    let activeColumns =
      VelouraSettings.isTablet && sliderColumnsTablet
        ? sliderColumnsTablet
        : sliderColumns;
  ...
  initSlider() {
    ...
    this.slider = new window.VelouraTheme.Slider(this);
...
class QuantityInput extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.quantityUpdateUnsubscriber = VelouraEvents.subscribe(
      PUB_SUB_EVENTS.quantityUpdate,
      this.validateQtyRules.bind(this)
    );
    this.quantityBoundriesUnsubscriber = VelouraEvents.subscribe(
      PUB_SUB_EVENTS.quantityBoundries,
      this.setQuantityBoundries.bind(this)
    );
    this.quantityRulesUnsubscriber = VelouraEvents.subscribe(
  ...
  setQuantityBoundries({ data: { sectionId, productId } }) {
    ...
    VelouraEvents.emit(PUB_SUB_EVENTS.quantityUpdate, undefined);
  }
    ...
    constructor() {
      ...
      this.addEventListener(
        ...
        debounce((event) => {
          ...
          fetch(`${VelouraSettings.routes.cart_update_url}`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
class CartDiscount extends HTMLElement {
  ...
  handleFormSubmit(e) {
    ...
    if (discounts.includes(newDiscountCode)) {
      this.displayFormErrors(window.VelouraCartStrings.duplicateDiscountError);
      return;
    }
    ...
    fetch(window.VelouraSettings.routes.cart_update_url, config)
      ...
      .then(async (parsedState) => {
        if (
          ...
          this.couponEl.value = '';
          this.displayFormErrors(window.VelouraCartStrings.applyDiscountError);
          return;
        }
    ...
    handleSave(e) {
      ...
      if (currentTime > Date.now()) {
        ...
        window.VelouraTheme.Notification.show({
          ...
          method: "appendChild",
          type: "warning",
          message: window.VelouraStrings.noSelectDeliveryTime,
        });
      } else {
        localStorage.removeItem(this.deliveryCodeKey);
        window.VelouraTheme.Notification.show({
          ...
          method: "appendChild",
          type: "warning",
          message: window.VelouraStrings.invalidDeliveryTime,
        });
      }
    }
    handleClose(e) {
...
class ImageComparisonSlider extends HTMLElement {
  ...
  handleMouseMove(e) {
    ...
    const x = this.horizontal
      ? VelouraSettings.isRTL ? this.getBoundingClientRect().right - event.pageX : event.pageX - this.offsetLeft
...
class ProductRecentlyViewed extends HTMLElement {
  constructor() {
    ...
    if (isStorageSupported("local")) {
      ...
      const cookieName = "veloura-recently-viewed";
...
class MediaHero extends HTMLDivElement {
  ...
  init() {
    const isAdapt = VelouraSettings.isMobile
