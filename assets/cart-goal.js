if (!customElements.get("f-cart-goal")) {
  class FCartGoal extends HTMLElement {
    constructor() {
      ...
      this.money_format = window.VelouraSettings.money_format;
      this.enableConfetti = this.dataset.enableConfetti === "true";
    }

    connectedCallback() {
      this.updateCartGoal(Number(this.dataset.cartTotal));
      window.VelouraEvents.subscribe(PUB_SUB_EVENTS.cartUpdate, (state) => {
        this.updateCartGoal(state.items_subtotal_price);
      });
      window.VelouraEvents.subscribe(PUB_SUB_EVENTS.cartUpdate, async () => {
        const newCart = await window.VelouraCartHelpers.getState();
        if (newCart) {
    ...
    showConfetti() {
      ...
      if (!this.confettiShow && (cartDrawer || cartTemplate)) {
        setTimeout(() => {
          if (cartDrawer) {
            new window.VelouraTheme.Confetti(cartDrawer);
          }
          if (cartTemplate) {
            new window.VelouraTheme.Confetti(document.body);
          }
