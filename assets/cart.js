...
class CartItems extends HTMLElement {
  ...
  connectedCallback() {
    this.cartUpdateUnsubscriber = window.VelouraEvents.subscribe(
      PUB_SUB_EVENTS.cartUpdate,
      async (parsedState) => {
        if (window.VelouraSettings.template === "cart") {
          this.renderContents(parsedState);
        }
        const cart = await VelouraCartHelpers.getState();
        if (cart) {
          this.cart = cart;
          VelouraCartHelpers.updateCartCount(cart.item_count);
        }
      }
    );
  }
  ...
  updateQuantity(line, quantity, name) {
    ...
    fetch(`${VelouraSettings.routes.cart_change_url}`, {
      ...fetchConfig(),
      ...{ body },
    })
      .then((response) => {
      ...
      .then((state) => {
        ...
        if (
          ...
          if (typeof updatedValue === "undefined") {
            message = window.VelouraStrings.cartError;
          } else {
            message = window.VelouraStrings.quantityError.replace(
              "[quantity]",
              updatedValue
            );
          }
      ...
      .catch(() => {
        ...
        if (errors) {
          errors.textContent = window.VelouraStrings.cartError;
        }
      })
      .finally(() => {
        this.loading.finish();
