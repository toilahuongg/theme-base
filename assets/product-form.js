      ...
      onSubmitHandler(evt) {
        ...
        if (missing && missing.length > 0) {
          return this.Notification.show({
            ...
            method: "appendChild",
            type: "warning",
            message: window.VelouraStrings.requiredField,
          });
        }
        ...
        const { VelouraSettings, VelouraStrings } = window;
        fetch(`${VelouraSettings.routes.cart_add_url}`, config)
          .then((response) => response.json())
          .then((response) => {
            if (response.status) {
              window.VelouraEvents.emit(PUB_SUB_EVENTS.cartError, {
                source: "product-form",
            ...
            if (
              !VelouraSettings.cart.openDrawerWhenAdded ||
              VelouraSettings.cart.cartType === "page"
            ) {
              this.Notification.show({
                target: this.errorWrapper ? this.errorWrapper : document.body,
                method: "appendChild",
                type: "success",
                message: VelouraStrings.item_added,
                last: 3000,
                sticky: this.notificationType === "toast",
              });
            }
            if (VelouraSettings.cart.cartType === "page") {
              VelouraCartHelpers.getState().then((state) => {
                VelouraCartHelpers.updateCartCount(state.item_count);
              });
            }

            this.error = false;
            window.VelouraEvents.emit(PUB_SUB_EVENTS.cartUpdate, response);
