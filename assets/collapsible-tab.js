	...
	class CollapsibleTab extends HTMLElement {
		...
		init() {
			...
			if (this.getAttribute('open') === 'true') {
				...
				this.fire("tabOpened")
				window.VelouraEvents.emit(`ON_COLLAPSIBLE_TAB_OPENED`, this)
			} else {
		...
		collapse() {
			const endTransition = (ev) => {
				...
				window.VelouraEvents.emit(`ON_COLLAPSIBLE_TAB_CLOSED`, this)
				this.setCollapsedAria()
				this.classList.add(this.collapsedClass)
		...
		open() {
			...
			window.VelouraEvents.emit(`ON_COLLAPSIBLE_TAB_SELECTED`, this)
			this.expand()
		...
		close() {
			...
			window.VelouraEvents.emit(`ON_COLLAPSIBLE_TAB_UNSELECTED`, this)
			this.collapse()
