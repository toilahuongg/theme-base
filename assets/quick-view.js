if (typeof QuickView === "undefined") {
  const { VelouraStyles, VelouraScripts } = window;
  class QuickView {
    constructor() {
      this.modal = null;
      ...
      window.VelouraEvents.subscribe(PUB_SUB_EVENTS.cartUpdate, () => {
    ...
    fetchHtml(productHandle) {
      loadAssets(
        [VelouraStyles.product, VelouraStyles.volumePricing],
        "quick-view-assets"
      );
      fetchSection("product-quickview", {
        url: `${window.VelouraSettings.base_url}products/${productHandle}`,
      })
        .then((html) => {
          ...
          loadAssets(
            [
              VelouraScripts.productInfo,
              VelouraScripts.productMedia,
              VelouraScripts.variantsPicker,
              VelouraScripts.volumePricing,
            ],
            "variants-picker",
            () => {
